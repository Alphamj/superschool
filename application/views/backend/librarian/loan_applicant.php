    <?php echo form_open(base_url() . 'index.php?librarian/loan_applicant/create' , 
      array('class' => 'form-horizontal form-groups-bordered validate','target'=>'_top'));?>
           ...
           <?php $librarian = $this->db->get_where('librarian', array('librarian_id' => $this->session->userdata('login_user_id')))->result_array();
foreach ($librarian as $row):
    ?>
			<input type="hidden" class="form-control" name="staff_name" value="<?php echo $row['librarian_id'];?>" / required>
...
<hr>
                  ...
                  <div class="form-group">
                    <div class="col-sm-offset-1 col-sm-11">
                        <button type="submit" class="btn btn-orange btn-sm btn-icon icon-left"><i class="entypo-plus"></i><?php echo get_phrase('apply_now');?></button>
                    </div>
                  </div>
                    <?php echo form_close();?>
        </div>
