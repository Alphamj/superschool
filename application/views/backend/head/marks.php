...
td {font-family: Arial, sans-serif;font-size: 12px;padding: 8px;border: 1px solid #E6E9ED;border-width: 1px;overflow: hidden;word-break: normal;color: #000;}
 ...
 <div class="x_panel tablu" >
            ...
            <div class="tab-pane  <?php if(!isset($edit_data) && !isset($personal_profile) && !isset($academic_result) )echo 'active';?>" id="list">
				<center>
                <?php echo form_open(base_url() . 'index.php?head/manage_marks');?>
...
<!-- change link -->	<?php echo form_open(base_url() . 'index.php?head/marks/' . $exam_id . '/' . $class_id);?>
                    ...
                    <tbody>
						...
						foreach($students as $row):
							...
							<tr>
								...
								<td class="tg-yw4l"><input type="text" class="class_score form-control" value="<?php echo $row2['mark_total'];?>"  name="mark_total_<?php echo $row['subject_id'];?>" id="mark_total_<?php echo $row['subject_id'];?>" onchange="class_score_change('<?php echo $row['subject_id'];?>')" readonly = 'true'></td>
								<td class="tg-yw4l">
...
<!-- Junior Secondary -->
     ...
     <!-- change link -->	<?php echo form_open(base_url() . 'index.php?head/marks/' . $exam_id . '/' . $class_id);?>
							<tr>
								...
								<td class="tg-yw4l"><input type="text" class="class_score form-control" value="<?php echo $row2['mark_total'];?>"  name="mark_total_<?php echo $row['subject_id'];?>" id="mark_total_<?php echo $row['subject_id'];?>" onchange="exam_score_change('<?php echo $row['subject_id'];?>')" readonly = 'true'></td>
								<td class="tg-yw4l">
									<select  class="class_score form-control"  name="effort_marks_<?php echo $row['subject_id'];?>" >
										<option value="A" <?php if($row2['effort_marks'] =='A'){ echo "selected";}?>>A</option>
										<option value="B" <?php if($row2['effort_marks'] =='B'){ echo "selected";}?>>B</option>
	...
	<table cellpadding="0" cellspacing="0" border="0" class="tg">
			...
			?>
				...
				<tbody>
					...
					<tr>
						<td  colspan = "5">Keeps school rules and rgulations.</td>
						<td>
							<select  class="class_score form-control" name="R13">
								<option value="A" <?php if($row['R13'] =='A'){ echo "selected";}?>>A</option>
								<option value="B" <?php if($row['R13'] =='B'){ echo "selected";}?>>B</option>
...
<!-- Primary -->
				<?php } else if (strpos($class_type, 'primary') !== false || strpos($class_type, 'pri') !== false){ ?>
					...
					<table cellpadding="0" cellspacing="0" border="0" class="tg" style="width: 71%;display: inline-block;vertical-align: top;">
						...
						<tr>
							...
							<td colspan="3" style="text-align: center;">C.A 2<br>20</td>
							 ...
							 foreach($students as $row):
								 ...
								 ?>
									 ...
									 ?>
										 ...
										 <tr>
											 <td><?php echo $i; ?></td>
											 <td colspan="12"><?php echo  $this->crud_model->get_subject_name_by_ids('subject',$row2['subject_id']);?></td>
				...
				<?php $student_grades = $this->db->get_where('primary_student_grade',array('exam_id' => $exam_id ,
                                                        'class_id' => $class_id , 
                                                        'student_id' => $student_id,
                                                        'session_year'=>$get_system_settings[17]['description']))->result_array(); ?>
				...
				<tbody>
					...
					<tr>
						...
						<td><input type ="radio" name="punctuality_grade" value="A" <?php if($student_grades[0]['punctuality_grade'] =='A'){ echo "checked"; } ?> ></td>
...
<?php }else if (strpos($class_type, 'nursery') !== false || strpos($class_type, 'nur') !== false || strpos($class_type, 'toddler') !== false){ ?>
		...
		<table>
			...
			<?php /*
				////CREATE THE MARK ENTRY ONLY IF NOT EXISTS////
				$students	=	$this->crud_model->get_subjects_by_class01();
				foreach($students as $row):
					$verify_data = array('exam_id' => $exam_id ,'class_id' => $class_id ,'nursub_id' => $row['nursub_id'] , 
										'student_id' => $student_id,'session_year'=>$get_system_settings[17]['description']);
					$query = $this->db->get_where('nursery_student_marks' , $verify_data);
		...
		<table style="width:100%; vertical-align: bottom;" cellpadding="0" cellspacing="0" border="0" class="tg">
			...
			?>
					...
					<tr>
						...
						<td><input class="class_score6 form-control" type="text" value="<?php echo $row['Attendance'];?>" name="Attendance">Example: Attendance / No of times school opened</td>
						<input type="hidden" name="teach_sign" value="<?php echo $teach_sign[0]['teacher_id'] . '.' . 'jpg' ?>" />
						<input type="hidden" name="head_sign" value="<?php echo $head_sign[0]['head_id'] . '.' . 'jpg' ?>" />					</tr>
				<?php endforeach; ?>
	...
	<?php if($class_id == ''):?>
			<div class="alert alert-danger" align="center">Add Subject To Selected Class First&nbsp;&nbsp;
				<a href="<?php echo base_url();?>index.php?head/subject/"><button type="button" class="btn btn-orange btn-icon icon-left btn-sm"><i class="entypo-plus"></i>Add</button></a>
			</div>
		<?php endif;?>
		<h5 id="error_message"  align="center" style="color: red;display: none;margin-left: 50px;"><strong style="color:#FF0000">CA must be less or equal to 30 and exam score must be less than or equal to (70 for SS 1 & 2) or (100 for SS3) </strong></h5>
