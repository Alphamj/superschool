...
td {font-family: Arial, sans-serif;font-size: 12px;padding: 8px;border: 1px solid #E6E9ED;border-width: 1px;overflow: hidden;word-break: normal;color: #000;}
 ...
 <div class="x_panel tablu" >
		...
		<?php 
			...
			echo form_open(base_url() . 'index.php?teacher/score_sheet');?>
			<div class="col-md-3">
			...
			<table cellpadding="0" cellspacing="0" class="tg" style="width:100%; margin:auto;">
				...
				<?php
					...
					foreach($stud as $row2):
						if ($class_id > 28 && $class_id < 32 ){
							$marks1	=	$this->db->get_where('mark' , array('class_id' => 29, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks2	=	$this->db->get_where('mark' , array('class_id' => 30, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks3	=	$this->db->get_where('mark' , array('class_id' => 31, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks = array_merge($marks1,$marks2,$marks3);
						}
						 elseif ($class_id > 31 && $class_id < 35 ){
							$marks1	=	$this->db->get_where('mark' , array('class_id' => 32, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks2	=	$this->db->get_where('mark' , array('class_id' => 33, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks3	=	$this->db->get_where('mark' , array('class_id' => 34, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks = array_merge($marks1,$marks2,$marks3);
						}
						 elseif ($class_id > 34 && $class_id < 37 ){
							$marks1	=	$this->db->get_where('mark' , array('class_id' => 35, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks2	=	$this->db->get_where('mark' , array('class_id' => 36, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks3	=	$this->db->get_where('mark' , array('class_id' => 37, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks = array_merge($marks1,$marks2,$marks3);
						}
						$avgtmk = $this->db->get_where('subavg',array('exam_id' => $exam_id,'class_id'=>$class_id ,'session_year'=>$sessoin_id))->result_array();
						...
						if ($class_id > 28 && $class_id < 32 ){
						 ...
						 $average	=	$this->db->get_where('average' , $verify_datas)->result_array();
		...
		<?php }else if (strpos($class_type, 'junior secondary') !== false || strpos($class_type, 'jss') !== false){ ?>
			<table cellpadding="0" cellspacing="0" border="0" class="tg" style="width: 100%;display: inline-block;vertical-align: top;    overflow: auto;">
				...
				<?php
					...
					foreach($stud as $row2):
						if ($class_id > 19 && $class_id < 23 ){
							$marks1	=	$this->db->get_where('mark' , array('class_id' => 20, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks2	=	$this->db->get_where('mark' , array('class_id' => 21, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks3	=	$this->db->get_where('mark' , array('class_id' => 22, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks = array_merge($marks1,$marks2,$marks3);
						}
						 elseif ($class_id > 22 && $class_id < 26 ){
							$marks1	=	$this->db->get_where('mark' , array('class_id' => 23, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks2	=	$this->db->get_where('mark' , array('class_id' => 24, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks3	=	$this->db->get_where('mark' , array('class_id' => 25, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks = array_merge($marks1,$marks2,$marks3);
						}
						 elseif ($class_id > 25 && $class_id < 29 ){
							$marks1	=	$this->db->get_where('mark' , array('class_id' => 26, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks2	=	$this->db->get_where('mark' , array('class_id' => 27, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks3	=	$this->db->get_where('mark' , array('class_id' => 28, 'student_id' => $row2['student_id'],'exam_id' => $exam_id, 'session_year' => $sessoin_id))->result_array();
							$marks = array_merge($marks1,$marks2,$marks3);
						}
