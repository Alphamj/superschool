...
class Parents extends CI_Controller
{
   ...
   function view_student($student_id = '') {
    ...
    /**********SEARCH STUDENTS AND VIEW STUDENT MARKS *******************/
    
    function midterm_result($student_id = '', $param2 = '', $sparam3 = ''){
        if ($this->session->userdata('parent_login') != 1)
            redirect('login', 'refresh');
         ...
         if ($this->input->post('operation') == 'selection'){
			...
			if ($page_data['student_id'] > 0 && $page_data['student_id'] > 0 ){
				redirect(base_url() . 'index.php?parents/midterm_result/' . $page_data['student_id'].'/'.$page_data['exam_id'].'/'.$page_data['sessoin_id'], 'refresh');
            }
            else {
				$this->session->set_flashdata('info', 'please_student_name');
				redirect(base_url() . 'index.php?parents/midterm_result/', 'refresh');
			}
        }
            
        $page_data['sessoin_id'] = $sparam3;
		$page_data['student_id'] = $student_id;
		$page_data['exam_id'] = $param2;
		$page_data['page_info'] = 'midterm_result';

		$page_data['page_name'] = 'midterm_result';
		$page_data['page_title'] = get_phrase('view_student_marks');
		$this->load->view('backend/index', $page_data);
}
    ...
    function search_student($student_id = '', $param2 = '', $sparam3 = ''){
        if ($this->session->userdata('parent_login') != 1)
         ...
         if ($this->input->post('operation') == 'selection'){
			...
			if ($page_data['student_id'] > 0 && $page_data['student_id'] > 0 ){
				redirect(base_url() . 'index.php?parents/search_student/' . $page_data['student_id'].'/'.$page_data['exam_id'].'/'.$page_data['sessoin_id'], 'refresh');
            }
            else {
				$this->session->set_flashdata('info', 'please_student_name');
				redirect(base_url() . 'index.php?parents/search_student/', 'refresh');
			}
        }
