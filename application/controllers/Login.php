...
//        exit($path);
    ...
    function ajax_login() {
        ...
        if ($login_status == 'success') {
            ...
            if ($this->session->userdata('head_login') == 1){
				$response['redirect_url'] = base_url() . 'index.php?head/dashboard';
            }
            if ($this->session->userdata('formtutor_login') == 1){
				$response['redirect_url'] = base_url() . 'index.php?formtutor/dashboard';
			}
			if ($this->session->userdata('librarian_login') == 1){
				$response['redirect_url'] = base_url() . 'index.php?librarian/dashboard';
			}
			if ($this->session->userdata('accountant_login') == 1){
				 $response['redirect_url'] = base_url() . 'index.php?accountant/dashboard';
			}
			if ($this->session->userdata('hostel_login') == 1){
				 $response['redirect_url'] = base_url() . 'index.php?hostel/dashboard';
			}
			if ($this->session->userdata('student_login') == 1){
    ...
    function validate_login($email = '', $password = '') {
        ...
        // Checking login credential for head
        $query = $this->db->get_where('head', $credential);
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $this->session->set_userdata('head_login', '1');
            $this->session->set_userdata('head_id', $row->head_id);
            $this->session->set_userdata('login_user_id', $row->head_id);
            $this->session->set_userdata('name', $row->name);
            $this->session->set_userdata('login_type', 'head');
            return 'success';
        }

        // Checking login credential for formtutor
        $query = $this->db->get_where('formtutor', $credential);
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $this->session->set_userdata('formtutor_login', '1');
            $this->session->set_userdata('formtutor_id', $row->head_id);
            $this->session->set_userdata('login_user_id', $row->head_id);
            $this->session->set_userdata('name', $row->name);
            $this->session->set_userdata('login_type', 'formtutor');
            return 'success';
        }
    ...
    function ajax_forgot_password()
    {
        ...
        if ($query->num_rows() > 0) 
        {
         ...
         // Checking credential for head
         $query = $this->db->get_where('head' , array('email' => $email));
         if ($query->num_rows() > 0) 
         {
             $reset_account_type     =   'head';
             $this->db->where('email' , $email);
             $this->db->update('head' , array('password' => $new_password));
             $resp['status']         = 'true';
         }

        // Checking credential for formtutor
        $query = $this->db->get_where('formtutor' , array('email' => $email));
        if ($query->num_rows() > 0) 
        {
            $reset_account_type     =   'formtutor';
            $this->db->where('email' , $email);
            $this->db->update('formtutor' , array('password' => $new_password));
            $resp['status']         = 'true';
        }
