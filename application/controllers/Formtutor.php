...
class Formtutor extends CI_Controller
{ 
    ...
    /***default functin, redirects to login page if no formtutor logged in yet***/
    public function index()
    {
        if ($this->session->userdata('formtutor_login') != 1)
            redirect(base_url() . 'index.php?login', 'refresh');
        if ($this->session->userdata('formtutor_login') == 1)
            redirect(base_url() . 'index.php?formtutor/dashboard', 'refresh');
    }
    ...
    function dashboard()
    {
        ...
        $page_data['page_title'] = 'formtutor_dashboard';
			 ...
			 if ($this->input->post('operation') == 'selection') 
	{
		...
		{
            redirect(base_url() . 'index.php?formtutor/student_information/' . $page_data['class_id'], 'refresh');
        } 
		else 
		{
            $this->session->set_flashdata('info', 'please_select_class');
            redirect(base_url() . 'index.php?formtutor/student_information/', 'refresh');
        }
    }
    ...
    function student_marksheet_print_view($student_id , $exam_id) {
        ...
        $this->load->view('backend/formtutor/student_marksheet_print_view', $page_data);
    }
        ...
        if ($this->input->post('operation') == 'selection') 
	{
        ...
        if ($page_data['class_id'] > 0 ) 
		{
            redirect(base_url() . 'index.php?formtutor/academic_syllabus/' . $page_data['class_id'], 'refresh');
        } 
		else 
		{
            $this->session->set_flashdata('info', 'please_select_class');
            redirect(base_url() . 'index.php?formtutor/academic_syllabus/', 'refresh');
        }
    }
	 ...
	 function nursery_student_question($param1 = '', $param2 = '')
    {
        ...
        if ($this->input->post('operation') == 'insert') {
            ...
            if ($data['exam_id'] > 0 && $data['class_id'] > 0 && $data['subject_id'] > 0) {
                ...
                redirect(base_url() . 'index.php?formtutor/nursery_student_question/', 'refresh');
            } else {
   ...
   function marks0($exam_id = '', $class_id = '', $student_id = '')
   {
       ...
       if ($this->input->post('operation') == 'selection') {
           ...
           if ($page_data['exam_id'] > 0 && $page_data['class_id'] > 0 && $page_data['student_id'] > 0) {
               redirect(base_url() . 'index.php?formtutor/marks0/' . $page_data['exam_id'] . '/' . $page_data['class_id'] . '/' . $page_data['student_id'], 'refresh');
           } else {
               $this->session->set_flashdata('mark_message', 'Choose exam, class and subject');
               redirect(base_url() . 'index.php?formtutor/marks0/', 'refresh');
           }
       }
       if ($this->input->post('operation') == 'update') {
           ...
           if($this->input->post('class_types') != 'nursery'){
               $data3['subject_total_marks'] = round($total_subs);
               $this->db->where('student_id', $student_id);
               $this->db->where('class_id', $class_id);
               $this->db->where('exam_id', $exam_id);
               $this->db->where('session_year', $session_year);
           ...
           //code for update vacation date
           $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
           redirect(base_url() . 'index.php?formtutor/marks0/' . $this->input->post('exam_id') . '/' . $this->input->post('class_id') . '/' . $this->input->post('student_id'), 'refresh');
       }
       $page_data['exam_id']    = $exam_id;
       $page_data['class_id']   = $class_id;
       $page_data['student_id'] = $student_id;
   ...
   function marks($exam_id = '', $class_id = '', $student_id = '')
   {
       ...
       if ($this->input->post('operation') == 'selection') {
           ...
           if ($page_data['exam_id'] > 0 && $page_data['class_id'] > 0 && $page_data['student_id'] > 0) {
               redirect(base_url() . 'index.php?formtutor/marks/' . $page_data['exam_id'] . '/' . $page_data['class_id'] . '/' . $page_data['student_id'], 'refresh');
           } else {
               $this->session->set_flashdata('mark_message', 'Choose exam, class and subject');
               redirect(base_url() . 'index.php?formtutor/marks/', 'refresh');
           }
       }
       if ($this->input->post('operation') == 'update') {
           $subjects = $this->db->get_where('class_subject' , array('class_id' => $class_id))->result_array();
           ...
           foreach($subjects as $row) {
               if($this->input->post('class_types') == 'primary'){
                   $data['ca_1']      = $this->input->post('ca_1_' . $row['subject_id']);
                   $data['cw']      = $this->input->post('cw_' . $row['subject_id']);
                   $data['ca_2']      = $this->input->post('ca_2_' . $row['subject_id']);
                   $data['project_score']      = $this->input->post('project_score_' . $row['subject_id']);
                   $data['ca_marks']      = $this->input->post('ca_marks_' . $row['subject_id']);
                   $data['mark_obtained']      = $this->input->post('mark_obtained_' . $row['subject_id']);
                   $data['mark_total']      = $this->input->post('mark_total_' . $row['subject_id']);
                   $data['remark']      = $this->input->post('remark_' . $row['subject_id']);
                   ...
                   $tot = $data['ca_1']+$data['cw']+$data['ca_2']+$data['project_score']+$data['mark_obtained'];
                   $total_subs +=$tot;
           ...
           //code for update vacation date
           $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
           redirect(base_url() . 'index.php?formtutor/marks/' . $this->input->post('exam_id') . '/' . $this->input->post('class_id') . '/' . $this->input->post('student_id'), 'refresh');
       }
       $page_data['exam_id']    = $exam_id;
       $page_data['class_id']   = $class_id;
       $page_data['student_id'] = $student_id;
	...
	function tabulation_sheet($class_id = '', $exam_id = '', $student_id = '', $sessoin_id = '') {
		...
		if ($this->input->post('operation') == 'selection') {
			...
			if ($page_data['exam_id'] > 0 && $page_data['class_id'] > 0 && $page_data['student_id'] > 0 ) {
				redirect(base_url() . 'index.php?formtutor/tabulation_sheet/' . $page_data['class_id'] . '/' . $page_data['exam_id'] . '/' . $page_data['student_id'].'/'.$page_data['sessoin_id'], 'refresh');
			} else {
				$this->session->set_flashdata('mark_message', 'Choose class and exam');
				redirect(base_url() . 'index.php?formtutor/tabulation_sheet/', 'refresh');
			}
		}
		$page_data['exam_id'] = $exam_id;
		$page_data['class_id'] = $class_id;
	...
	function tabulation_sheet_print_view($class_id, $exam_id,$sessoin_id) {
		...
		$page_data['exam_id'] = $exam_id;
		$page_data['sessoin_id'] = $sessoin_id;
		$this->load->view('backend/formtutor/tabulation_sheet_print_view', $page_data);
    }
    ...
    function score_sheet($class_id = '', $exam_id = '', $sessoin_id = '') {
		...
		if ($this->input->post('operation') == 'selection') {
			...
			if ($page_data['exam_id'] > 0 && $page_data['class_id'] > 0) {
				redirect(base_url() . 'index.php?formtutor/score_sheet/' . $page_data['class_id'] . '/' . $page_data['exam_id'] . '/' .$page_data['sessoin_id'], 'refresh');
			} else {
				$this->session->set_flashdata('mark_message', 'Choose class and exam');
				redirect(base_url() . 'index.php?formtutor/score_sheet/', 'refresh');
			}
		}
		$page_data['exam_id'] = $exam_id;
		$page_data['class_id'] = $class_id;
	...
	function tabulation_sheet_print_view_control($class_id, $exam_id,$sessoin_id) {
		...
		$page_data['exam_id'] = $exam_id;
		$page_data['sessoin_id'] = $sessoin_id;
		redirect(base_url() . 'index.php?formtutor/tabulation_sheet_print_view/' . $page_data['class_id'] . '/' . $page_data['exam_id'].'/'.$page_data['sessoin_id'], 'refresh');
	}
	...
	function tabulation_sheet_print_view_single($class_id, $exam_id,$sessoin_id,$student_id){
		...
		$page_data['sessoin_id'] = $sessoin_id;
		$page_data['student_id'] = $student_id;
		$this->load->view('backend/formtutor/tabulation_sheet_print_view_single', $page_data);
	}
	...
	function tabulation_sheet_print_single_control($class_id, $exam_id,$sessoin_id,$student_id){
		...
		$page_data['sessoin_id'] = $sessoin_id;
		$page_data['student_id'] = $student_id;
		redirect(base_url() . 'index.php?formtutor/tabulation_sheet_print_view_single/' . $page_data['class_id'] . '/' . $page_data['exam_id'].'/'.$page_data['sessoin_id'].'/'.$student_id, 'refresh');
    }
...
function tabulation_sheet_midterm($class_id = '', $exam_id = '', $student_id = '', $sessoin_id = '') {
    ...
    if ($this->input->post('operation') == 'selection') {
        ...
        if ($page_data['exam_id'] > 0 && $page_data['class_id'] > 0 && $page_data['student_id'] > 0 ) {
            redirect(base_url() . 'index.php?formtutor/tabulation_sheet_midterm/' . $page_data['class_id'] . '/' . $page_data['exam_id'] . '/' . $page_data['student_id'].'/'.$page_data['sessoin_id'], 'refresh');
        } else {
            $this->session->set_flashdata('mark_message', 'Choose class and exam');
            redirect(base_url() . 'index.php?formtutor/tabulation_sheet_midterm/', 'refresh');
        }
    }
    $page_data['exam_id'] = $exam_id;
    $page_data['class_id'] = $class_id;
...
function tabulation_sheet_midterm_print_view($class_id, $exam_id,$sessoin_id) {
    ...
    $page_data['exam_id'] = $exam_id;
    $page_data['sessoin_id'] = $sessoin_id;
    $this->load->view('backend/formtutor/tabulation_sheet_midterm_print_view', $page_data);
}
...
function tabulation_sheet_midterm_print_view_control($class_id, $exam_id,$sessoin_id) {
    ...
    $page_data['exam_id'] = $exam_id;
    $page_data['sessoin_id'] = $sessoin_id;
    redirect(base_url() . 'index.php?formtutor/tabulation_sheet_midterm_print_view/' . $page_data['class_id'] . '/' . $page_data['exam_id'].'/'.$page_data['sessoin_id'], 'refresh');
}
...
function tabulation_sheet_midterm_print_view_single($class_id, $exam_id,$sessoin_id,$student_id){
    ...
    $page_data['sessoin_id'] = $sessoin_id;
    $page_data['student_id'] = $student_id;
    $this->load->view('backend/formtutor/tabulation_sheet_midterm_print_view_single', $page_data);
}
...
function tabulation_sheet_midterm_print_single_control($class_id, $exam_id,$sessoin_id,$student_id){
    ...
    $page_data['sessoin_id'] = $sessoin_id;
    $page_data['student_id'] = $student_id;
    redirect(base_url() . 'index.php?formtutor/tabulation_sheet_midterm_print_view_single/' . $page_data['class_id'] . '/' . $page_data['exam_id'].'/'.$page_data['sessoin_id'].'/'.$student_id, 'refresh');
}
    ...
    function backup_restore($operation = '', $type = '')
    {
        ...
        if ($operation == 'restore') {
            ...
            redirect(base_url() . 'index.php?formtutor/backup_restore/', 'refresh');
        }
        if ($operation == 'delete') {
    ...
    function manage_profile($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'update_profile_info') {
            ...
            $this->db->where('formtutor_id', $this->session->userdata('formtutor_id'));
            $this->db->update('formtutor', $data);
            move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/formtutor_image/' . $this->session->userdata('formtutor_id') . '.jpg');
            $this->session->set_flashdata('flash_message', get_phrase('account_updated'));
            redirect(base_url() . 'index.php?formtutor/manage_profile/', 'refresh');
        }
        if ($param1 == 'change_password') {
            ...
            $current_password = $this->db->get_where('formtutor', array(
                'formtutor_id' => $this->session->userdata('formtutor_id')
            ))->row()->password;
            if ($current_password == $data['password'] && $data['new_password'] == $data['confirm_new_password']) {
                $this->db->where('formtutor_id', $this->session->userdata('formtutor_id'));
                $this->db->update('formtutor', array(
        ...
        $page_data['edit_data']  = $this->db->get_where('formtutor', array(
            'formtutor_id' => $this->session->userdata('formtutor_id')
        ))->result_array();
        $this->load->view('backend/index', $page_data);
    }
    ...
    function class_routine($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            redirect(base_url() . 'index.php?formtutor/class_routine/', 'refresh');
        }
    ...
    {
        if($this->session->userdata('formtutor_login')!=1)redirect('login' , 'refresh');
        ...
        $this->session->set_flashdata('flash_message', get_phrase('data_updated'));
        redirect(base_url() . 'index.php?formtutor/manage_attendance/' . $date . '/' . $month . '/' . $year . '/' . $class_id . '/' . $section_id, 'refresh');
    }
    $page_data['date'] = $date;
    ...
    redirect(base_url() . 'index.php?formtutor/manage_attendance/' . $date . '/' . $this->input->post('class_id') . '/' . $this->input->post('section_id'), 'refresh');
}

function attendance_report_view() {
    redirect(base_url() . 'index.php?formtutor/attendance_report/' . $this->input->post('class_id') . '/' . $this->input->post('section_id') . '/' . $this->input->post('month') . '/' . $this->input->post('year'), 'refresh');
}
...
/**********MANAGE LOAN APPLICATIONS *******************/
    function loan_applicant($param1 = '', $param2 = '' , $param3 = '')
    {
		...
		if ($param1 == 'create') {
            ...
            redirect(base_url() . 'index.php?formtutor/loan_applicant' , 'refresh');
        }
    ...
    function assignment($param1 = '', $param2 = '' , $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            redirect(base_url() . 'index.php?formtutor/assignment' , 'refresh');
        }
		if ($param1 == 'do_update') {
             $data['timestamp']     = $this->input->post('timestamp');
            $data['title']         = $this->input->post('title');
            ...
            $this->db->update('assignment', $data);
			 $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?formtutor/assignment/'.$data['assignment_id'], 'refresh');
			}
    ...
    function examquestion($param1 = '', $param2 = '' , $param3 = '')
    {
      if ($this->session->userdata('formtutor_login') != 1)
        ...
        if ($param1 == 'create') {
            ...
            redirect(base_url() . 'index.php?formtutor/examquestion' , 'refresh');
        }
		if ($param1 == 'do_update') {
             $data['timestamp']     = $this->input->post('timestamp');
            $data['subject_id']         = $this->input->post('subject_id');
            ...
            $this->db->update('examquestion', $data);
			 $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?formtutor/examquestion'.$data['examquestion_id'], 'refresh');
			}
    ...
    function noticeboard($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            redirect(base_url() . 'index.php?formtutor/noticeboard/', 'refresh');
        }
        if ($param1 == 'do_update') {
            $data['notice_title']     = $this->input->post('notice_title');
            $data['notice']           = $this->input->post('notice');
    ...
    function help_link($param1 = '', $param2 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?formtutor/help_link', 'refresh');
        }
        if ($param1 == 'do_update') {
            $data['title']         = $this->input->post('title');
            $data['link'] = $this->input->post('link');
    ...
    function document($do = '', $document_id = '')
    {
        ...
        if ($do == 'upload') {
            ...
            redirect(base_url() . 'formtutor/manage_document', 'refresh');
        }
        if ($do == 'delete') {
    ...
    function study_material($task = "", $document_id = "")
    {
        ...
        if ($task == "create")
        {
            ...
            redirect(base_url() . 'index.php?formtutor/study_material' , 'refresh');
        }
        ...
        if ($task == "delete")
        {
            ...
            redirect(base_url() . 'index.php?formtutor/study_material');
        }
    ...
    function message($param1 = 'message_home', $param2 = '', $param3 = '') {
        ...
        if ($param1 == 'send_new') {
            ...
            redirect(base_url() . 'index.php?formtutor/message/message_read/' . $message_thread_code, 'refresh');
        }

        if ($param1 == 'send_reply') {
            $this->crud_model->send_reply_message($param2);  //$param2 = message_thread_code
            $this->session->set_flashdata('flash_message', get_phrase('message_sent!'));
            redirect(base_url() . 'index.php?formtutor/message/message_read/' . $param2, 'refresh');
        }
