...
class accountant extends CI_Controller
{
    ...
    if ($param1 == 'create_mass_invoice') {
        if (($this->input->post('student_id'))) {
            foreach ($this->input->post('student_id') as $id) {
                ...
                $data2['title'] = $this->input->post('title');
                $data2['description'] = $this->input->post('description');
                $data2['payment_type'] = $this->input->post('description');
                $data2['method'] = $this->input->post('method');
                $data2['amount'] = $this->input->post('amount_paid');
                $data2['timestamp'] = strtotime($this->input->post('date'));
	...
	if($this->input->post('operation') =='selection'){
		...
		redirect(base_url() . 'index.php?accountant/expense/'.$expence_category_id, 'refresh');
	}
    if ($param1 == 'delete') {
        $this->db->where('payment_id', $param2);
...
/**********MANAGE LOAN APPLICATIONS *******************/
    ...
    function loan_approval($param1 = '', $param2 = '' , $param3 = '')
    {
        ...
        $page_data['page_title'] = get_phrase('view_status');
